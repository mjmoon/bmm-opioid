data {
  ...
  real<lower=0, upper=1> u[n_time]; // other mortality rates
  // misc
  vector[n_time] pop; // population counts
  int pop0; // population at t-1
}
parameters {
  ...
  real init_s1_logitp; // {logit(s10 / pop0)}
  real init_s2_logitp; // {logit(s20 / pop0)}
}
transformed parameters {
  ...
  for (n in 1:n_time) {
    ...
    lambda_trn[n][1, 1] = (1 - p_mean[n][1]) * (1 - u[n]);
    lambda_trn[n][2, 2] = (1 - p_mean[n][2]) * (1 - p_mean[n][3]) * (1 - u[n]);
    lambda_trn[n][1, 2] = p_mean[n][1] * (1 - u[n]);
    lambda_trn[n][2, 3] = p_mean[n][2] * (1 - u[n]);
    lambda_trn[n][2, 4] = (1 - p_mean[n][2]) * p_mean[n][3] * (1 - u[n]);
  }
  // set initial state prevalence
  lambda_cnt[1][1] = inv_logit(init_s1_logitp) * pop0;
  lambda_cnt[1][2] = inv_logit(init_s2_logitp) * pop0;
  lambda_cnt[1][3] = 0;
  lambda_cnt[1][4] = 0;
  for (n in 1:n_time) {
...
generated quantities {
  ...
  real p2_hat[n_time];
  real p3_hat[n_time];
  real x_hat_per_thousand[n_time];
  real y_hat_per_thousand[n_time];
  real r1_hat_per_thousand[n_time];
  real r2_hat_per_thousand[n_time];
  real w_hat_per_thousand[n_time];
  ...
  for (n in 1:n_time) {
    ...
    p2_hat[n] = p_mean[n][2];
    p3_hat[n] = p_mean[n][3];
    x_hat_per_thousand[n] = x_hat[n] / pop[n] * 1000;
    y_hat_per_thousand[n] = y_hat[n] / pop[n] * 1000;
    r1_hat_per_thousand[n] = r1_hat[n] / pop[n] * 1000;
    r2_hat_per_thousand[n] = r2_hat[n] / pop[n] * 1000;
    w_hat_per_thousand[n] = w_hat[n] / pop[n] * 1000;
  }
}
