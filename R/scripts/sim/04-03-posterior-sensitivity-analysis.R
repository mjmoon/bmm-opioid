...
surveyed <- readRDS(file.path(data_path, "surveyed.rds"))$counts |>
  ...
  slice(-c(1, n + 1))
# read fitted object
for (f in dir(data_path, pattern = "-sa-", full.names = TRUE)) {
  suffix <- sub("\\.rds", "", sub(".*fit-", "", f))
  adj <- as.numeric(sub("_", "", stringr::str_extract(suffix, "_\\d+"))) / 100
  fit <- readRDS(f)
  ...
  # plot posterior
  plot_path <- file.path("results", "sim", "sensitivity-analysis", 
                         paste0(suffix, "-00-posterior-single-"))
  plot_posterior_prior_density(
    posts, "siggamma",
    function(x) ifelse(x > 0, 2 * dnorm(
      x, 
      sd = sigma_sigma_gamma * ifelse(grepl("gamma", suffix), adj, 1)), 
      0),
  ...
  plot_posterior_prior_density(
    posts, "sigdelta",
    function(x) ifelse(x > 0, 2 * dnorm(
      x, sd = sigma_sigma_delta * ifelse(grepl("delta", suffix), adj, 1)), 
      0),
  ...
  plot_posterior_prior_density(
    ...
    function(x) ifelse(
      abs(x) <= 1,
      dnorm(x, sd = .5) / (pnorm(1, sd = 1) - pnorm(-1, sd = 1)),
      0
    ),
  ...
  plot_path <- file.path("results", "sim", "sensitivity-analysis", 
                         paste0(suffix, "-01-posterior-over-time-"))
  post_theme <- theme_minimal() +
    theme(plot.margin = margin(l = 1, r = 0, t = 0.5, b = 0, unit = "in"))
  
  r1_prior <- list(
    dfun = d_log_norm,
    mean = r0[1] * ifelse(grepl("r0", suffix), adj, 1),
    sigma_sigma = sigma_sigma_gamma * ifelse(grepl("gamma", suffix), adj, 1)
  )
  ...
  r2_prior <- list(
    dfun = d_log_norm,
    mean = r0[2] * ifelse(grepl("r0", suffix), adj, 1),
    sigma_sigma = sigma_sigma_gamma * ifelse(grepl("gamma", suffix), adj, 1)
  )
  ...
  p1_prior <- list(
    dfun = d_logit_norm,
    mean = p0[1] * ifelse(grepl("p0", suffix), adj, 1),
    sigma_sigma = sigma_sigma_delta * ifelse(grepl("delta", suffix), adj, 1)
  )
  plot_posterior_over_time(p1_posts, prior = p1_prior, both_races = FALSE,
                           ...
                           xlimits = c(1, n), xbreaks = c(1, n),
                           save = FALSE) +
    geom_point(aes(y = time, x = tr1/s1), data = surveyed,
               shape = 23, size = 5, fill = "darkgrey") 
  ggsave(file = paste0(plot_path, "p1.png"), 
         dpi = 450, width = 8, height = 6, unit = "in")
  
  p2_prior <- list(
    dfun = d_logit_norm,
    mean = p0[2] * ifelse(grepl("p0", suffix), adj, 1),
    sigma_sigma = sigma_sigma_delta * ifelse(grepl("delta", suffix), adj, 1)
  )
  plot_posterior_over_time(p2_posts, prior = p2_prior, both_races = FALSE,
                           ...
                           xlimits = c(1, n), xbreaks = c(1, n),
                           save = FALSE) +
    geom_point(aes(y = time, x = tr2/s2), data = surveyed,
               shape = 23, size = 5, fill = "darkgrey") 
  ggsave(file = paste0(plot_path, "p2.png"), 
         dpi = 450, width = 8, height = 6, unit = "in")
  
  p3_prior <- list(
    dfun = d_logit_norm,
    mean = p0[3] * ifelse(grepl("p0", suffix), adj, 1),
    sigma_sigma = sigma_sigma_delta * ifelse(grepl("delta", suffix), adj, 1)
  )
