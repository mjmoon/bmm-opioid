...
surveyed <- readRDS(file.path(data_path, "surveyed.rds"))$counts |>
  ...
  slice(-c(1, n + 1))
# read fitted object
fit <- readRDS(file.path(data_path, "fit-ratio-estimate.rds"))
...
# plot posterior
plot_path <- file.path("results", "sim", "ratio-estimate", "00-posterior-single-")
...
plot_path <- file.path("results", "sim", "ratio-estimate", "01-posterior-over-time-")
