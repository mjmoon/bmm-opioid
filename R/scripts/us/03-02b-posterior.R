...
pop0 <- readRDS(file.path(data_path, "mort.rds")) |>
  ...
  pull(Population)
# read fitted object
fit <- readRDS(file.path(data_path, "fit_black.rds"))
...
r2_posts <- get_post_intervals(fit, "r2_hat_per_thousand",
  ...
  rename(year = time)
# plot posterior
plot_path <- file.path("results", "us", "black", "01-posterior-single-")
...
plot_path <- file.path("results", "us", "black", "02-posterior-over-time-")
