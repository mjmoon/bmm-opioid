...
# fit ----------------------------------------
message("Fitting for US white population between 2015-2019. -----")
fit <- fit_opioid_transitions(
  ...
  model_name = "Non-hispanic white US 2015-2019 with survey",
  control = control, warmup = warmup, iter = iter
)
saveRDS(fit, file.path("data", "us", "fit_white.rds"))
timestamp <- format(Sys.time(), format = "%Y-%m-%d %H:%M")
message(paste0("----------", timestamp, "----------"))
message(paste0("------------------------------------"))
rm(list = ls())
